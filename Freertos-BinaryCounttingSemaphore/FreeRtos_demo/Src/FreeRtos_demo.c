...
void task1(void * pvParameters)
{
	...
	while (1)
	{
		...
		usbprintf("任务1:按键按下释放二值信号量，按键双击释放计数信号量，计数值增加\r\n");
		for(uint8_t i=0;i<KEY_COUNT;i++)
		{
			...
			if(Key_Check(i,KEY_DOUBLE))
			{
				if(i==0)
				{
					...
					/*释放信号量计数值+1*/
					res = xSemaphoreGive(CoutingSemaphore);
					if(res == pdPASS)
					{
						usbprintf("计数信号量释放成功！\r\n");
...
void task2(void * pvParameters)
{
	...
	BaseType_t res_Binary;
	BaseType_t res_Counting;
	BaseType_t current_count = 0;

	while (1)
	{

		usbprintf("任务2:获取二值信号量和计数信号量\r\n");

		res_Binary = xSemaphoreTake(BinarySemaphore, portMAX_DELAY);
		if(res_Binary ==pdPASS)
		{
			usbprintf("获取二值信号量成功！\r\n");
		...
		/*获取信号量计数值-1*/
		res_Counting = xSemaphoreTake(CoutingSemaphore, portMAX_DELAY);
		if(res_Counting==pdPASS)
		{
			usbprintf("获取计数信号量成功！\r\n");
        ...
        current_count = uxSemaphoreGetCount(CoutingSemaphore);
		usbprintf("当前计数值:%d\r\n",current_count);
		vTaskDelay(1000);
