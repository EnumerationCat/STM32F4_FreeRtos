...
#define EVENT_BIT_0 (1 << 0) // 定义事件位0
#define EVENT_BIT_1 (1 << 1) // 定义事件位1
	...
	while (1)
	{
		...
		for(uint8_t i=0;i<KEY_COUNT;i++)
		{
			if(Key_Check(i,KEY_SINGLE))
			{
				...
				{
					usbprintf("按键%d被单击\r\n",i);
					//xResult = xTaskNotifyGive(task2_TaskHandle); // 发送事件通知给任务2，触发任务2的执行,模拟信号量
					//xResult = xTaskNotify(task2_TaskHandle,i, eSetValueWithOverwrite); // 发送事件通知给任务2，并携带数值i，触发任务2的执行，模拟队列消息

					xResult = xTaskNotify(task2_TaskHandle,EVENT_BIT_0, eSetBits); // 发送事件通知给任务2，并设置事件位0，触发任务2的执行，模拟事件组
					if(xResult == pdPASS)
					{
						//usbprintf("向任务2通知发送按键%d成功！\r\n",i);
						usbprintf("任务1:向任务2发送事件通知成功！\r\n");
					}
				}
			...
			if(Key_Check(i,KEY_DOUBLE))
			{
				...
				{
					usbprintf("按键%d被双击\r\n",i);
					xResult = xTaskNotify(task2_TaskHandle,EVENT_BIT_1, eSetBits); // 发送事件通知给任务2，并设置事件位1，触发任务2的执行，模拟事件组
					if(xResult == pdPASS)
					{
						//usbprintf("向任务2通知发送按键%d成功！\r\n",i);
						usbprintf("任务1:向任务2发送事件通知成功！\r\n");
...
void task2(void * pvParameters)
{
	...
	while (1)
	{
		...
		usbprintf("任务2:收到通知\r\n");
		/*
			pdTRUE表示接收后清除通知值,相当于二值信号量
			pdFALSE表示接收后不清除通知值,相当于计数信号量
			portMAX_DELAY表示无限等待
			当任务1发送通知时，任务2会被唤醒并继续执行
		*/
		//ulNotificationValue = ulTaskNotifyTake(pdFALSE, portMAX_DELAY);
		xTaskNotifyWait(0x00000000,		//不清除任何位
						0xffffffff,		//清除所有位
			  			&ulEventGroupValue,
			   			portMAX_DELAY);

		usbprintf("任务2:收到事件通知值 %d\r\n", ulEventGroupValue);
		
		if(ulEventGroupValue & EVENT_BIT_0)
		{
			usbprintf("任务2:收到事件通知，事件位0被设置\r\n");
		}
		if(ulEventGroupValue & EVENT_BIT_1)
		{
			usbprintf("任务2:收到事件通知，事件位1被设置\r\n");
		}
