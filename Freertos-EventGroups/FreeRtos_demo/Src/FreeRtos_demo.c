	...
	while (1)
	{
		...
		for(uint8_t i=0;i<KEY_COUNT;i++)
		{
			if(Key_Check(i,KEY_SINGLE))
			{
				...
				{
					usbprintf("按键%d被单击\r\n",i);
					xEventGroupSetBits(event_group_handle, EVENT_BIT_0); // 设置事件位0，通知其他任务按键0被单击
			...
			if(Key_Check(i,KEY_DOUBLE))
			{
				...
				{
					usbprintf("按键%d被双击\r\n",i);
					xEventGroupSetBits(event_group_handle, EVENT_BIT_1); // 设置事件位1，通知其他任务按键0被双击
...
void task2(void * pvParameters)
{
	...
	while (1)
	{
		...
		usbprintf("任务2:等待按键标志\r\n");
		event_bits = xEventGroupWaitBits(event_group_handle,
			EVENT_BIT_0|EVENT_BIT_1, // 等待事件位0或事件位1
			pdTRUE, // 等待后清除事件位
			pdFALSE, // 等待所有事件位（pdFALSE表示等待任意一个事件位）
			portMAX_DELAY); // 无限等待
		usbprintf("任务2:收到按键标志 %#x\r\n", event_bits);
		vTaskDelay(500);
