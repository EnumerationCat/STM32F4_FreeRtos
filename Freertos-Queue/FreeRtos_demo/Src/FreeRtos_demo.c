...
void freertos_start(void)
{
	...
	if(queue1!=NULL)
	{
		usbprintf("queue1创建成功！");
	}else{
		usbprintf("queue1创建失败！");
	}
	...
	if(big_queue!=NULL)
	{
		usbprintf("big_queue创建成功！");
	}else{
		usbprintf("big_queue创建失败！");
	}
	...
	while (1)
	{
			...
			for(uint8_t i=0;i<KEY_COUNT;i++)
		{
			if(Key_Check(i,KEY_SINGLE))
			{
				...
				{
					usbprintf("按键%d单击\r\n",i);
					err = xQueueSend(queue1, &i, portMAX_DELAY);

					if (err != pdTRUE)
					{
						usbprintf("queue1队列发送失败!\r\n");
					}else{
						usbprintf("queue1队列发送成功!\r\n");
					}
				}
			...
			if(Key_Check(i,KEY_DOUBLE))
			{
				if(i==0)
				{
					...
					if (err != pdTRUE)
					{
						usbprintf("big_queue队列发送失败!\r\n");
					}else{
						usbprintf("big_queue队列发送成功!\r\n");
					}
...
void task2(void * pvParameters)
{
	...
	while (1)
	{
		...
		usbprintf("任务2:读取小队列\r\n");
		err = xQueueReceive(queue1, &keynum, portMAX_DELAY);//读不到数据会进入阻塞态
        if (err != pdTRUE)
        {
            usbprintf("queue1队列读取失败!\r\n");
        }else{
            usbprintf("queue1读取队列成功!\r\n");
			usbprintf("读取数据：%d\r\n", keynum);
        }
...
void task3(void * pvParameters)
{
	...
	while (1)
	{
		...
		usbprintf("任务3:读取大队列\r\n");
		err = xQueueReceive(big_queue, &buf, portMAX_DELAY);//读不到数据会进入阻塞态
        if (err != pdTRUE)
        {
            usbprintf("big_queue队列读取失败!\r\n");
        }
        else
        {
			usbprintf("big_queue队列读取成功!\r\n");
			 usbprintf("读取数据：%s\r\n", buf);
        }
